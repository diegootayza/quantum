...
Quantum is a full-stack AI conversation platform built with Nuxt 4, featuring role-based user management (USER/ADMIN), subscription management, and multi-model AI integration through the Vercel AI SDK gateway.
...
Primary entities: User, Conversation, Message, Instruction, Category, Attachment, Subscription, Service, Setting. User roles are USER or ADMIN. Messages store content in `parts` (Json array).

### AI Integration

Multi-provider via Vercel AI SDK gateway. Models configured in `useModels()` composable. Conversation streaming uses `createUIMessageStream`. Tool support includes image generation via Gemini.
...
Required: `DATABASE_URL`, `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_URL`, plus AI provider API keys.
